...
import com.fengmap.FMDemoNavigationAdvance.location.StepLocation;
...
//                Log.i("zjx", "隔2s的步数差：" + stepSub);
                double degreeSub=(30.0/*30.0是指南针指向*/+90.0+360.0-lastDegree)%360; //相对地球竖直向上↑的角度差 角度从逆时针算
//                Log.i("zjx","degreeSub:"+degreeSub);
                ...
                if((stepSub=(step-stepLocation.lastAMapPoint.getStep()))!=0){
                    ...
                    if(stepSub<0){
                        ...
                        double sinStepY = lenStep * Math.sin(dia) / 400.0;//y+-
                        double cosStepX = lenStep * Math.cos(dia) / 400.0;//x+-
                        xMap = stepLocation.lastAMapPoint.getX() + cosStepX;
                        yMap = stepLocation.lastAMapPoint.getY() + sinStepY;
                        //更新到CustomApplcation__暂时先不更新
